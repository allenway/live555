...
RHARTSPServer* 
RHARTSPServer::createNew(UsageEnvironment& env, Port ourPort,
			     UserAuthenticationDatabase* authDatabase,
			     unsigned reclamationTestSeconds) {
  int ourSocket = setUpOurSocket(env, ourPort);
  if (ourSocket == -1) return NULL;

  return new RHARTSPServer(env, ourSocket, ourPort, authDatabase, reclamationTestSeconds);
}

RHARTSPServer::RHARTSPServer(UsageEnvironment& env, int ourSocket,
				     Port ourPort,
				     UserAuthenticationDatabase* authDatabase, unsigned reclamationTestSeconds)
  : DynamicRTSPServer(env, ourSocket, ourPort, authDatabase, reclamationTestSeconds) {
}

RHARTSPServer::~RHARTSPServer() {
}

static ServerMediaSession* createNewSMS(UsageEnvironment& env,
					char const* fileName, FILE* fid); // forward

ServerMediaSession* RHARTSPServer
::lookupServerMediaSession(char const* streamName, Boolean isFirstLookupInSession) {
  // Next, check whether we already have a "ServerMediaSession" for this file:
  ServerMediaSession* sms = RTSPServer::lookupServerMediaSession(streamName);
  Boolean smsExists = sms != NULL;
//step1检测rtsp地址是否为代理地址
  printf("client request rtsp stream name:%s\n",streamName);
  if(!strcmp("proxyStream",streamName))
  {
    return sms;
  }
//step2动态创建本地媒体流
